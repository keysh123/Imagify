import React from 'react'
import { assets } from '../assets/assets';

const Header = () => {
  return (
    <>
      <div className='flex flex-col items-center justify-center text-center my-20'>
        <div className='text-stone-500 inline-flex  gap-2 text-center bg-white px-6 py-1 rounded-full border border-neutral-500'>
          <p>
            Best text to Image Converter
          </p>
          <img src={assets.star_icon} alt="" />
        </div>
        <h1 className='text-4xl max-w-[300px] sm:text-7xl sm:max-w-[590px] mx-auto mt-10 text-center'>Turn text to <span className='text-blue-600'>image</span>, in seconds.</h1>

        <p className='text-center mx-auto mt-5 max-w-xl'>Unleash your creativity with AI. Turn your imagination into visual art in seconds - Just type and watch the magic happen.</p>

        <button className='sm:text-lg text-white bg-black w-auto mt-8 px-12  py-2.5 flex items-center gap-2 rounded-full'>Genrerate Images <img className='h-6' src={assets.star_group} alt="" /></button>

        <div className='flex justify-center gap-3 mt-16 flex-wrap'>
          {
            Array(6).fill('').map((item,index)=>{
              return(
                <>
                {index%2 == 0 ?
                <img className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' key={index} 
                src={assets.sample_img_2} alt="" width={70} /> :
                <img className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' key={index} 
                src={assets.sample_img_1} alt="" width={70} />
                }
                </>
              )
            })
          }
        </div>
        <p className='text-neutral-600 mt-2'>
          Genterated images from imagify
        </p>
      </div>
    
    </>
  )
}

export default Header